...
def chat(
    ...
    if tools:
        import copy
        tools_clone = copy.deepcopy(tools)
...
def run_tool(tool_name, tool_input):
    if tool_name == "get_current_datetime":
        ...
        # In a production environment, implement an actual Web Search API call here.
    ...
    while True:
        response = chat(
            ...
            system=[
                {
                    "type": "text",
                    "text": "You are a helpful AI assistant specialized in analyzing long documents. Provide precise and accurate answers based only on the provided context.",
                ...
                if hasattr(block, "citations") and block.citations:
                    ...
                    for citation in block.citations:
                        if isinstance(citation, dict):
                            doc_title = citation.get(
                                "document_title",
                                citation.get("document_index", "Unknown Document"),
                            )
                            ...
                            doc_title = getattr(
                                citation,
                                "document_title",
                                getattr(citation, "document_index", "Unknown Document"),
                            )
                            start_page = getattr(citation, "start_page_number", "N/A")
                            cited_text = getattr(citation, "cited_text", "N/A")

                        print("Document :", doc_title)
                        print("Page :", start_page)
                        print("Text :", cited_text)
                        print("---------------------")
            elif block.type == "tool_use":
                print(f"Using Tool: {block.name}")
            elif block.type == "server_tool_use":
                print("Server Tool Use:", block.input)
            elif block.type == "code_execution_tool_result":
                print("Code Execution Result:", block.content)
            else:
...
if os.path.getsize(csv_path) > 32 * 1024 * 1024:
...
try:
    ...
    print("Error: Your Anthropic SDK version does not support the Files API. Please upgrade.")
    exit(1)
...
add_user_message(
    ...
    [
        ...
        {"type": "text", "text": "Analyze this CSV data and generate a summary graph."},
    ],
)
