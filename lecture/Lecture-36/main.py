...
with open("lecture/Lecture-36/report.md", "r", encoding="utf-8") as f:
    text = f.read()
...
def generate_embedding(text, input_type, model="voyage-3-large"):
    result = voyage_client.embed([text], model=model, input_type=input_type)
    return result.embeddings[0]

# --- Lecture 36: Semantic Search (Cosine Similarity) ---
...
print("Top Matches\n")
for i, (score, chunk) in enumerate(similarities[:3], start=1):
    print(f"Match {i}")
